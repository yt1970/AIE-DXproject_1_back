...
概要
----
本ドキュメントは、AIE-DXproject のバックエンド（FastAPI + Celery + SQLAlchemy）に関する全体設計、コンポーネント構成、主要データモデル、主要フロー、運用・設定の要点をまとめたものです。CSV アップロードから LLM 分析、結果の蓄積および参照までの一連の処理を対象とします。
