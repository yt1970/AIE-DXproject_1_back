...
class LLMAnalysisResult(BaseModel):
    ...
    priority: str | None = Field(default=None, description="重要度 (high, medium, low)")
    fix_difficulty: str | None = Field(default=None, description="修正難易度 (easy, hard, none)")
    risk_level: str | None = Field(default=None, description="危険度ラベル (例: none, low, medium, high)")
...
class LLMClientConfig(BaseModel):
    ...
    base_url: str | None = Field(default=None, description="LLM APIのエンドポイントURL")
    model: str | None = Field(default=None, description="使用するモデル名")
    api_key: str | None = Field(default=None, description="APIキー")
    api_version: str | None = Field(default=None, description="APIバージョン (Azure向け)")
    organization: str | None = Field(default=None, description="OpenAI組織IDなど、必要であれば設定")
    ...
    extra_headers: dict[str, str] = Field(default_factory=dict, description="リクエストに追加で付与するヘッダ")
...
class LLMClient:
    ...
    def __post_init__(self) -> None:
        if self.config.require_external_api() and not self.config.base_url:
            raise ValueError("LLMClientConfig.base_url is required when provider is not 'mock'.")
        ...
        *,
        ...
        try:
            with httpx.Client(
                ...
                response = client.post(self.config.base_url, json=payload, headers=headers, params=params)
            ...
            raise LLMClientError(f"LLM API returned HTTP error: {exc.response.status_code} - {error_details}") from exc
        ...
        if normalized_warnings:
            result.warnings.extend([w for w in normalized_warnings if w not in result.warnings])
    ...
    def _extract_structured_payload(self, response: httpx.Response) -> dict[str, Any]:
        try:
            ...
            raise LLMResponseFormatError(f"LLM API returned non-JSON response: {text[:200]}") from exc
    ...
    def _unwrap_response_body(self, body: Any) -> dict[str, Any]:
        ...
        raise LLMResponseFormatError(f"Unexpected LLM response structure: {type(body).__name__}")
    ...
    def _extract_from_choices(self, choices: list[Any]) -> dict[str, Any]:
        ...
        if content is None and isinstance(first_choice.get("content"), list):
            ...
            content = "".join(part.get("text", "") for part in content_parts if isinstance(part, dict)).strip() or None
        ...
        if isinstance(content, str):
            ...
            try:
                ...
                raise LLMResponseFormatError("Failed to parse JSON content from LLM choice.") from exc

        if isinstance(content, list):
            concatenated = "".join(item.get("text", "") for item in content if isinstance(item, dict)).strip()
            cleaned = self._strip_code_fences(concatenated)
            try:
                return self._ensure_dict(json.loads(cleaned))
            except json.JSONDecodeError as exc:
                raise LLMResponseFormatError("Failed to parse JSON content from structured LLM messages.") from exc

        if isinstance(content, dict):
            return self._ensure_dict(content)

        raise LLMResponseFormatError(f"Unsupported message content type: {type(content).__name__}")
    ...
    def _ensure_dict(self, payload: Any) -> dict[str, Any]:
        if isinstance(payload, dict):
            return payload
        raise LLMResponseFormatError(f"LLM response is not a JSON object: {type(payload).__name__}")
