...
def get_lecture_detail(lecture_id: int, db: Annotated[Session, Depends(get_db)]) -> LectureDetailResponse:
    ...
    distributions = (
        ...
        .join(
            models.SurveyBatch,
            models.ScoreDistribution.survey_batch_id == models.SurveyBatch.id,
        )
        ...
        average_scores=(
            [
                AverageScoreItem(
                    ...
                    score=float(summary.avg_satisfaction_overall or 0),
                    full_mark=5,
                ),
                AverageScoreItem(
                    category="学習量",
                    category_key="learning_amount",
                    score=float(summary.avg_content_volume or 0),
                    full_mark=5,
                ),
                AverageScoreItem(
                    category="理解度",
                    category_key="comprehension",
                    score=float(summary.avg_content_understanding or 0),
                    full_mark=5,
                ),
                AverageScoreItem(
                    category="運営",
                    category_key="operations",
                    score=float(summary.avg_content_announcement or 0),
                    full_mark=5,
                ),
                AverageScoreItem(
                    category="講師満足度",
                    category_key="instructor_satisfaction",
                    score=float(summary.avg_instructor_overall or 0),
                    full_mark=5,
                ),
                AverageScoreItem(
                    category="時間使い方",
                    category_key="time_management",
                    score=float(summary.avg_instructor_time or 0),
                    full_mark=5,
                ),
                AverageScoreItem(
                    category="質問対応",
                    category_key="question_handling",
                    score=float(summary.avg_instructor_qa or 0),
                    full_mark=5,
                ),
                AverageScoreItem(
                    category="話し方",
                    category_key="speaking_style",
                    score=float(summary.avg_instructor_speaking or 0),
                    full_mark=5,
                ),
                AverageScoreItem(
                    category="予習",
                    category_key="preparation",
                    score=float(summary.avg_self_preparation or 0),
                    full_mark=5,
                ),
                AverageScoreItem(
                    category="意欲",
                    category_key="motivation",
                    score=float(summary.avg_self_motivation or 0),
                    full_mark=5,
                ),
                AverageScoreItem(
                    category="今後活用",
                    category_key="future_application",
                    score=float(summary.avg_self_future or 0),
                    full_mark=5,
                ),
            ]
            if summary
